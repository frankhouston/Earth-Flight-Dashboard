...
export const HUB_CITIES: readonly HubCity[] = [
  ...
  // URC (Urumqi), KHG (Kashgar), ALA (Almaty), and VCE (Venice)
  // has completed a proper Silk Road representation across the dashboard.
  // The route traces: XIY → URC → KHG → ALA → TAS → ASB → IKA → BGW → IST → VCE
  // ──────────────────────────────────────────────────────────────────────────────
  ...
  { code: 'TAS', name: 'Tashkent', country: 'UZ', lat: 41.2574, lon: 69.2907, volume: 0.3, type: 'mixed' }, // Visually offset north in MarkerSystem to avoid clutter with NQZ
